...
function addTags() {
  ...
  if (
    ...
    String(route.query?.runtimePageCode || '') !== ''
  ) {
    useTagsViewStore().setDashboardRuntimeView(route)
    return
  }
  if (name) {
...
function refreshSelectedTag(view) {
  if ((!view?.path || view.path === route.path) &&
    (route.meta?.dashboardRuntime || route.path.startsWith('/dashboard/runtime/'))) {
    window.dispatchEvent(new CustomEvent('dashboard:refresh-data', { detail: { path: route.path } }))
    return
  }
