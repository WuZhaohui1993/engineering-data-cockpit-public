  ...
  'tags-view',
  {
    ...
    actions: {
      ...
      updateVisitedView(view) {
        const current = this.visitedViews.find(item => item.path === view.path)
        if (current) {
          const meta = { ...current.meta, ...view.meta }
          const title = view.title || current.title || meta.title || 'no-name'
          Object.assign(current, view, { meta, title })
          saveVisitedViews(this.visitedViews)
        }
      },
      // 内部切页只更新当前入口的查询参数；独立设计/运行页签互不删除。
      setDashboardRuntimeView(view) {
      ...
      updateDashboardPageTitle({ path, pageName, mode }) {
        const current = this.visitedViews.find(item => item.path === path)
        const name = String(pageName || '').trim()
        const label = String(mode || '').trim()
        if (!current || !name || !label || current.meta?.dashboardRuntime === true) return
        current.title = `${name} · ${label}`
        current.meta = {
          ...current.meta,
          dashboardPageName: name,
          dashboardPageMode: label,
        }
        saveVisitedViews(this.visitedViews)
      },
      delRightTags(view) {
